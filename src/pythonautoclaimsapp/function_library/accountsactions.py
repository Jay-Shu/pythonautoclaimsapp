...
from toga.validators import LengthBetween
...
class UpdAccountClass:
    def __init__(self, cnxn, data, columns):
        ...
        self.validators = toga.validators.LengthBetween
    ...
    def updateAccount(self, widget):
        ...
        'ACCOUNT_TYPE'
        """
        accountNumLabel = toga.Label('Account Number', style=Pack(font=14))
        accountHonorificsLabel = toga.Label(
            'Account Honorifics', style=Pack(font=14))
        accountFNLabel = toga.Label('Account First Name', style=Pack(font=14))
        accountLNLabel = toga.Label('Account Last Name', style=Pack(font=14))
        accountSuffixLabel = toga.Label('Account Suffix', style=Pack(font=14))
        accountStreetAdd1Label = toga.Label(
            'Account Street Address 1', style=Pack(font=14))
        accountStreetAdd2Label = toga.Label(
            'Account Street Address 2', style=Pack(font=14))
        accountCityLabel = toga.Label('Account City', style=Pack(font=14))
        accountStateLabel = toga.Label('Account State', style=Pack(font=14))
        accountZIPLabel = toga.Label('Account ZIP', style=Pack(font=14))
        accountPOBoxLabel = toga.Label('Account PO Box', style=Pack(font=14))
        accountDateStartLabel = toga.Label(
            'Account Date Start', style=Pack(font=14))
        accountDateRenewalLabel = toga.Label(
            'Account Date Renewal', style=Pack(font=14))
        accountTypeLabel = toga.Label('Account Type', style=Pack(font=14))

        v = toga.validators.Number()

        accountNumText = toga.TextInput()  # This must be supplied.
        accountNumText.validators = LengthBetween(11, 11)
        accountNumText.validators = Contains('ICA')
        accountHonorificsText = toga.TextInput()
        accountHonorificsText.validators = LengthBetween(0, 16)
        accountFNText = toga.TextInput()
        accountFNText.validators = LengthBetween(0, 128)
        accountLNText = toga.TextInput()
        accountLNText.validators = LengthBetween(0, 128)
        accountSuffixText = toga.TextInput()
        accountSuffixText.validators = LengthBetween(0, 16)
        accountStreetAdd1Text = toga.TextInput()
        accountStreetAdd1Text.validators = LengthBetween(0, 128)
        accountStreetAdd2Text = toga.TextInput()
        accountStreetAdd2Text.validators = LengthBetween(0, 128)
        accountCityText = toga.TextInput()
        accountCityText.validators = LengthBetween(0, 128)
        accountStateText = toga.TextInput()
        accountStateText.validators = LengthBetween(11, 16)
        accountZIPText = toga.TextInput()
        accountZIPText.validators = v
        # accountZIPText.validators = LengthBetween(0,5)
        accountPOBoxText = toga.TextInput()
        accountPOBoxText.validators = LengthBetween(0, 128)
        accountDateStartText = toga.TextInput()
        accountDateStartText.validators = LengthBetween(0, 32)
        accountDateRenewalText = toga.TextInput()
        accountDateRenewalText.validators = LengthBetween(0, 32)
        accountTypeText = toga.TextInput()
        accountTypeText.validators = LengthBetween(0, 64)

        # We need to pass our value of False or True onto the next part. A def will be required for updating these properly each.
        # accountNumSwitch = toga.Switch('Null?',style=Pack(font=14),value=False,on_change=self._update_account_handler('ACCOUNT_NUM',accountNumText,accountsJson))
        accountHonorificsSwitch = toga.Switch(
            'Null?', style=Pack(font=14), value=False, on_change=None)
        accountFNSwitch = toga.Switch('Null?', style=Pack(
            font=14), value=False, on_change=None)
        ...
        # createAccountButton = toga.Button('Create Account',style=Pack(padding=5),on_press=self._create_account_callback)
        # deleteAccountsButton = toga.Button('Delete Account',style=Pack(padding=5),on_press=self._delete_account_callback)
...
class CreAccountClass:
   ...
   def createAccount(self, widget):
      ...
      cancelButton = toga.Button('Cancel', style=Pack(
          padding=5), on_press=self._cancel_handler(widet))
      accountsmenu.add(submitButton)
      accountsmenu.add(cancelButton)
...
class DeleteAccountClass:
   ...
   def deleteAccount(self, widget,accountsJson):
      accountsmenu = toga.Box(style=Pack(direction=COLUMN))
   ...
   def _delete_account_callback(self,widget,initJson):
...
def main():
    return UpdAccountClass(), CreAccountClass(), DeleteAccountClass()