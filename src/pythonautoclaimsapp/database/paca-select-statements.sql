...
CREATE PROCEDURE paca.getVehicles_v1
AS
BEGIN TRY
SELECT
VEHICLE_ACCOUNT_NUM,
...
CREATE PROCEDURE paca.getVehicles_v2
...
CREATE PROCEDURE paca.getVehicles_v3
@vehicleAccountNum NVARCHAR(11)
AS
BEGIN TRY
SELECT
VEHICLE_ACCOUNT_NUM,
	...
	VEHICLE_ADDRESS2_GARAGED
  FROM paca.VEHICLES
  WHERE VEHICLE_VIN = @vehicleAccountNum
END TRY
BEGIN CATCH
SELECT 
  ERROR_NUMBER() AS ErrorNumber
...
CREATE PROCEDURE paca.getPolicies_v1
AS
BEGIN TRY
SELECT
POLICIES_ID_ACTUAL,
...
CREATE PROCEDURE paca.getVehicleCoverages_v1
AS
BEGIN TRY
SELECT
...
SELECT 
  ...
  ,ERROR_SEVERITY() AS ErrorSeverity
  ,ERROR_STATE() AS ErrorState
  ,ERROR_PROCEDURE() AS ErrorProceduref
  ,ERROR_LINE() AS ErrorLine
  ,ERROR_MESSAGE() AS ErrorMessage;
END CATCH
...
CREATE PROCEDURE paca.getVehicleClaims_v1
AS
BEGIN TRY
SELECT
...
CREATE PROCEDURE paca.getVehicleClaims_v2
@vehicleClaimsIntCaseNum NVARCHAR(11)
AS
BEGIN TRY
SELECT
VEHICLE_CLAIMS_INTERNAL_CASE_NUMBER,
...
WHERE VEHICLE_CLAIMS_ACCOUNT_NUM = @vehicleClaimsIntCaseNum
END TRY
BEGIN CATCH
SELECT 
  ERROR_NUMBER() AS ErrorNumber
...
CREATE PROCEDURE paca.getVehicleClaims_v3
@vehicleVin NVARCHAR(32)
AS
BEGIN TRY
SELECT
VEHICLE_CLAIMS_INTERNAL_CASE_NUMBER,
	...
	VEHICLE_CLAIMS_TOW_COMPANY
FROM paca.VEHICLE_CLAIMS
WHERE VEHICLE_CLAIMS_VEHICLE_VIN = @vehicleVin
END TRY
BEGIN CATCH
SELECT 
  ERROR_NUMBER() AS ErrorNumber
...
CREATE PROCEDURE paca.getVehicleClaims_v4
@vehicleClaimsIntCaseNum NVARCHAR(11),
@vehicleVin NVARCHAR(32)
AS
BEGIN TRY
...
FROM paca.VEHICLE_CLAIMS
WHERE VEHICLE_CLAIMS_VEHICLE_VIN = @vehicleVin
AND VEHICLE_CLAIMS_ACCOUNT_NUM = @vehicleClaimsIntCaseNum
END TRY
BEGIN CATCH
SELECT 
  ERROR_NUMBER() AS ErrorNumber
